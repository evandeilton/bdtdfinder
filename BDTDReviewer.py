...
class EnhancedBDTDReviewer:
    ...
    def search_and_download(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Erro durante busca e download: {e}")
            return False
    ...
    def calculate_metrics(self, texts: List[str], dates: List[str]) -> ReviewMetrics:
        """
        Calcula métricas da revisão.
        
        Args:
            texts (List[str]): Lista de textos
            dates (List[str]): Lista de datas
            
        Returns:
            ReviewMetrics: Métricas calculadas
        """
        try:
            # Análise temporal
            parsed_dates = [
            ...
            } if parsed_dates else {"start": "N/A", "end": "N/A"}
            
            # Concatena textos para análise
            ...
            metrics = ReviewMetrics(
                ...
                quality_score=len(texts) / self.max_pages,  # Métrica simples de qualidade
                sentiment_score=self.analyze_sentiment(full_text),
                top_keywords=self.extract_keywords(full_text),
                cluster_info=self.cluster_documents(texts)
            )
    ...
    def run(self) -> Optional[str]:
        ...
        try:
            ...
            self.metrics = self.calculate_metrics(
                texts=df["results"].tolist(),
                dates=df["date"].tolist() if "date" in df.columns else []
            )
            
            if not self.metrics:
                logger.error("Falha no cálculo de métricas")
                return None
