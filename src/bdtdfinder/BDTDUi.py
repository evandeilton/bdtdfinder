"""BDTD Research Agent & Reviewer package - Interface de Usuário via Streamlit."""
...
class BDTDUi:
    """
    Classe auto executável para a interface do usuário do BDTD Research Agent & Reviewer.
    
    Ao ser instanciada, o construtor chama automaticamente o método `run_ui()`, 
    que configura e renderiza toda a interface via Streamlit.
    """
    ...
    def __init__(self):
        ...
        Chama o método responsável por inicializar a interface.
        """
        self.run_ui()
    ...
    def load_markdown_file(file_path):
        ...
        :return: Conteúdo do arquivo ou mensagem de erro em caso de exceção.
        """
        try:
    ...
    def get_pdf_download_link(pdf_path):
        """
        Gera um link HTML para download de um arquivo PDF.
        
        :param pdf_path: Caminho para o arquivo PDF.
        :return: String contendo o link para download do arquivo.
        """
    ...
    def find_pdfs(directory):
        """
        Procura por arquivos PDF em um diretório e retorna informações sobre eles.
        
        :param directory: Diretório onde serão buscados os PDFs.
        :return: Lista de dicionários com informações de cada PDF encontrado.
        """
        pdfs = []
    ...
    def run_ui(self):
        """
        Inicializa e renderiza a interface do usuário via Streamlit.
        
        Esse método contém toda a lógica de configuração da página, definição de CSS,
        construção da sidebar, área central, execução dos processos de revisão e exibição
        dos resultados.
        """
        ...
        def modelo_display(model_id):
            """
            Formata a exibição do modelo para a seleção.
            
            :param model_id: Identificador do modelo.
            :return: String com o nome do modelo e se é Grátis ou Pago.
            """
                ...
                <div class="centered-search">
                    <div>
                        ...
                        <p style="text-align: center; font-size: 1.1rem;">
                            O sistema realiza buscas automáticas na BDTD, processa os resultados usando IA e gera uma revisão de literatura estruturada.
                        </p>
                    </div>
                </div>
            """, unsafe_allow_html=True)
            ...
            with col_center:
                ...
                if iniciar:
                    if not research_theme:
                        ...
                        # Lista de curiosidades para exibição
                        curiosidades = [
        ...
        with main_container:
            ...
            with tabs[0]:
                if 'last_review' in st.session_state:
                    md_content = self.load_markdown_file(st.session_state.last_review)
                    ...
                    if latest_md:
                        md_content = self.load_markdown_file(latest_md[-1])
                        col1, col2 = st.columns([6, 1])
                        with col1:
                            st.markdown(md_content, unsafe_allow_html=True)
                        with col2:
                        ...
                        st.markdown("""
                            ### Como utilizar?
                            1. Digite o tema da sua pesquisa na área central.
                            2. Ajuste as opções no painel lateral.
                            3. Forneça sua chave de API da OpenRouter e escolha seu modelo.
                            4. Clique em **Iniciar Revisão** para iniciar o processo.
                        """)

            # Aba de PDFs Baixados
            with tabs[1]:
                if os.path.exists(output_dir):
                    pdfs = self.find_pdfs(output_dir)
                    if pdfs:
                        import pandas as pd
                        ...
                        for folder_name, folder_info in sorted(folders.items()):
                            with st.expander(f"📁 {folder_name} ({len(folder_info['pdfs'])} arquivos, {folder_info['total_size']:.2f} MB)"):
                                ...
                                for pdf_info in folder_info['pdfs']:
                                    st.markdown(
                                        f"- {self.get_pdf_download_link(pdf_info['path'])} ({pdf_info['size']:.2f} MB)",
                                        unsafe_allow_html=True
                                    )
...
if __name__ == "__main__":
    # Ao executar este arquivo diretamente, a classe é instanciada e a UI é inicializada automaticamente.
    BDTDUi()
